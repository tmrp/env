...
> **Running the version step locally?** The CHANGELOG is generated by
> `@changesets/changelog-github`, which calls the GitHub API to resolve PR and
> author links. CI provides the token automatically, but if you ever run
> `pnpm run version:packages` on your machine you must export a token first, or
> it aborts without writing anything:
>
> ```sh
> GITHUB_TOKEN=$(gh auth token) pnpm run version:packages
> ```
